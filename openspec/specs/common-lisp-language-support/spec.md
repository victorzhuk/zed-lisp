...
The extension SHALL provide a `textobjects.scm` that captures list forms and function/macro definitions as text objects for structural selection commands.

#### Scenario: Selecting a form as a text object

- **WHEN** a user invokes "select enclosing text object" inside a list form
- **THEN** the entire s-expression (including parentheses) is selected
